...
class TestExports:
    ...
    def test_export_synthese_sinp_csv(self, users):
        ...
        response = self.client.post(
            url_for("exports.getOneExportThread", id_export=1, export_format="csv"),
            json={"email": "test"},
        )
    ...
    def test_memory_profiling_get_one_export_api(self, users):
        return self.test_get_one_export_api(users)

    def test_memory_profiling_export_synthese_sinp_csv(self, users):
        return self.test_export_synthese_sinp_csv(users)
