...
format_map_transform = {
    Format.CSV: "COPY ({}) TO STDOUT WITH CSV HEADER DELIMITER ',';",  # noqa E501
    Format.JSON: "COPY (SELECT json_agg(t) FROM ({}) AS t) TO STDOUT;",  # noqa E501
    Format.RDF: ''
}
...
async def run(queue=queue, num_workers=num_workers):
    with psycopg2.connect(dsn) as db:
        with db.cursor() as cursor:
            cursor.execute(
                'SELECT e.label, l.id, l.format, e.selection FROM gn_exports.t_exports_logs l JOIN gn_exports.t_exports e ON e.id = l.id_export WHERE status=-2 ORDER BY id ASC;')  # noqa E501
